...
def add_record_to_workflow_audit_tbl(
    workflow_audit_item: dict, workflow_ddb_tbl: str
) -> None:
    ...
    try:
        ...
        put_item_into_dynamodb(workflow_ddb_tbl, workflow_audit_item)
...
def update_records_in_workflow_audit_tbl(
    ...
    try:
        ...
        update_dynamodb_items(workflow_ddb_tbl, [workflow_audit_item])
    except Exception as e:
        logger.error(
            "Error while updating93 record to the Workflow Audit Table : " + str(e)
        )
        raise


def add_record_in_file_audit_tbl(file_audit_item: dict, file_ddb_tbl: str) -> None:
    ...
    try:
        ...
        put_item_into_dynamodb(file_ddb_tbl, file_audit_item)
    except Exception as e:
        logger.error("Error while adding record to the File Audit Table : " + str(e))
        raise


def update_records_in_file_audit_tbl(file_audit_item: dict, file_ddb_tbl: str) -> None:
    """Updates item into Workflow Audit DynamoDB table
    ...
    try:
        ...
        # workflow_audit_item['execution_end_ts'] = datetime.utcnow().strftime("%m/%d/%Y %H:%M:%S")
        update_dynamodb_items(file_ddb_tbl, [file_audit_item])
    except Exception as e:
